...
export default function PhotosUploader({addedPhotos,myonChange}){
    ...
    function removePhoto(ev,filename){
        ev.preventDefault();
        myonChange([...addedPhotos.filter(photo => photo!== filename)]);
        ...
        <>
            ...
            <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {addedPhotos.length > 0 && addedPhotos.map(mypic => {
                    return (
                        <div className="h-32 flex relative" key={mypic}>
                            <img className="rounded-2xl w-full object-cover"
                                 src={"http://localhost:4000/upload/"+mypic}
                                 alt = ""/>
                            <button onClick={event => removePhoto(event,mypic)}
                                    className="absolute bottom-1 right-1 text-red-400 bg-black p-2 cursor-pointer bg-opacity-50 rounded-2xl">
                            ...
                            <button onClick={event => selectAsMainPhoto(event,mypic)}
                                    className="absolute bottom-1 left-1 text-red-400 bg-black p-2 cursor-pointer bg-opacity-50 rounded-2xl">
                                {mypic !== addedPhotos[0] && (
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
